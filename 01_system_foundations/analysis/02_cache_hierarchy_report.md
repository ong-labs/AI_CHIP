...
* **그래프 설정**: 가로 8, 세로 5인치의 크기로 설정하며, `plt.semilogx`를 사용하여 $x$축의 급격한 크기 변화를 효과적으로 보여줍니다.
* **성능 패턴**:
    * **L1/L2 Cache**: 약 $52 \sim 58$ $ns$ 수준의 최소 지연 시간을 유지하며, Python 오버헤드를 제외한 순수 로직이 상주하는 구간입니다.
    * **L3 transition**: 데이터 크기가 커짐에 따라 지연 시간이 약 $75$ $ns$ 수준으로 증가합니다.
    * **DRAM & TLB Penalty**: $64$ $MB$ 이상의 영역에서는 약 $110 \sim 150$ $ns$까지 지연 시간이 늘어나며, L3 캐시 용량 한계와 TLB 범위를 벗어나는 Random Access가 결합되어 최대 지연 시간이 관찰됩니다.
